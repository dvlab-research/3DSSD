...
def draw_lidar(pc, color=None, fig=None, bgcolor=(0,0,0), pts_scale=1, pts_mode='point', pts_color=None):
    ...
    if color is None: color = pc[:,2]
    # draw the points
    # x, y, z, s: s represents the scale of each point
...
def draw_gt_boxes2d(gt_boxes2d, fig, color=(1,1,1), line_width=1, draw_text=True, text_scale=(1, 1, 1), color_list=None):
    num = len(gt_boxes2d)
    for n in range(num):
        b = gt_boxes2d[n]
        ...
        for i in range(4):
            mlab.plot3d([b[i, 0], b[(i+1)%4, 0]], [b[i, 1], b[(i+1)%4, 1]], [b[i, 2], b[(i+1)%4, 2]], color=color, tube_radius=None, line_width=line_width, figure=fig)
    return fig
...
if __name__=='__main__':
    ...
    pc = np.fromfile('dataset/KITTI/object/training/velodyne/000000.bin', dtype=np.float32)
    pc = pc.reshape([-1, 4])
    pc = pc[:, :3]

...
REGISTER_OP("CubeSelectModified")
        .Input("xyz1: float32") // b, n, 3
        .Input("xyz2: float32") // b, m, 3
        .Output("idx: int32") // b, n, 8
        .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
...
void cubeSelectModifiedLauncher(int b, int n, int m, const float* xyz1, const float* xyz2, int* idx_out);
class CubeSelectModifiedOp : public OpKernel {
public:
    explicit CubeSelectModifiedOp(OpKernelConstruction * context): OpKernel(context) {}
    ...
    void Compute(OpKernelContext* context) override {
        ...
        const Tensor& xyz2_tensor = context->input(0);
        ...
        auto idx_out_flat = idx_out_tensor->flat<int>();
        int* idx_out = &(idx_out_flat(0));
        cubeSelectModifiedLauncher(b, n, m, xyz1, xyz2, idx_out);
    }
};
REGISTER_KERNEL_BUILDER(Name("CubeSelectModified").Device(DEVICE_GPU), CubeSelectModifiedOp);

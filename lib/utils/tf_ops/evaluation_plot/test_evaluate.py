...
from utils.tf_ops.evaluation_plot.tf_evaluate import evaluate
...
label_dir = os.path.join(ROOT_DIR, 'result/2018-10-03 17:51:27.269339/result')
...
for f_name in tqdm.tqdm(os.listdir(label_dir)):
    gt_label_name = os.path.join(dataset_dir, 'training', 'label_2', f_name)
    obj_detection_name.append(gt_label_name)

    label_name = os.path.join(label_dir, f_name)
    label_objects = kitti_util.read_label(label_name) 
    ...
    for i in range(objects_num):
        ...
        obj_detection[13] = label_object.score 
...
print(obj_detection_list.shape)
obj_detection_name = np.array(obj_detection_name, dtype=np.string_)
obj_detection_num = np.array(obj_detection_num, dtype=np.int)
model_name = np.array(['test', 'test'], dtype=np.string_)
print(len(obj_detection_name))
print(obj_detection_num.shape)
print(obj_detection_name)
print(model_name)
        
precision_img, aos_img, precision_ground, aos_ground, precision_3d, aos_3d = evaluate(obj_detection_list, obj_detection_name, obj_detection_num, model_name) 

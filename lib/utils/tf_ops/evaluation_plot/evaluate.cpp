...
bool eval(const string* model_name, const float* dets, const string* names, const int* num_list, const int num_images, float* precision_image, float* aos_image, float* precision_ground, float* aos_ground, float* precision_3d, float* aos_3d){
  // set some global parameters
  initGlobals();
  ...
  //     vector<tGroundtruth> gt = groundtruth[i];
  //     for (int j=0; j<gt.size(); j++) {
  //         std::cout << gt[j].box.type << " " << gt[j].box.x1 << " " << gt[j].box.y1 << std::endl;
  //     }
  //     std::cout << "------------------------------------" << std::endl;
  // }
  ...
  //     vector<tDetection> dt = detections[i];
  //     for (int j=0; j<dt.size(); j++) {
  //         std::cout << dt[j].box.type << " " << dt[j].box.x1 << " " << dt[j].box.y1 << " " << dt[j].thresh << std::endl;
  //     }
  //     std::cout << "------------------------------------" << std::endl;
  // }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, compute_aos_ground, imageBoxOverlap, precision[0], aos[0], aos_grd[0], EASY, IMAGE);
    ...
    // std::cout << "saved result Done ---" << std::endl;
    if (c == 0){
      std::cout << model_name[0] << " " << CLASS_NAMES[c] << std::endl;
      saveAndPlotPlots(model_name[0], CLASS_NAMES[c] + "_detection", CLASS_NAMES[c], precision, 0);
    }
    // std::cout << "First Evaluation Done ----" << std::endl;
    if(compute_aos){
      saveAndPlotPlots(model_name[0], CLASS_NAMES[c] + "_orientation", CLASS_NAMES[c], aos, 1);
      fclose(fp_ori);
    }
    // std::cout << "First Evaluation Done ----" << std::endl;
  }
  // std::cout << "Calculating detection ground Done !!!" << std::endl;
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    vector<double> precision[3], aos[3], aos_grd[3];
    if (eval_ground[c]) {
      eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, compute_aos_ground, groundBoxOverlap, precision[0], aos[0], aos_grd[0], EASY, GROUND);
    ...
    // std::cout << "image evaluation done !" << std::endl;
    if (c == 0)
      saveAndPlotPlots(model_name[0], CLASS_NAMES[c] + "_detection_ground", CLASS_NAMES[c], precision, 0);
  }
  //  std::cout << "Calculating detection bev Done !!!" << std::endl;
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    vector<double> precision[3], aos[3], aos_grd[3];
    if (eval_3d[c]) {
      eval_class(fp_det, fp_ori, cls, groundtruth, detections, compute_aos, compute_aos_ground, box3DOverlap, precision[0], aos[0], aos_grd[0],  EASY, BOX3D);
    ...
    std::cout << "image evaluation done !" << std::endl;
    if (c == 0)
      saveAndPlotPlots(model_name[0], CLASS_NAMES[c] + "_detection_3d", CLASS_NAMES[c], precision, 0);
  }
